		...
		try {
			...
			switch (error.code) {
				...
				case 'auth/invalid-email':
					...
					break;
				default:
					setError('حدث خطأ أثناء إرسال رابط إعادة تعيين كلمة المرور. الرجاء المحاولة مرة أخرى');
			}
		} finally {
			setLoading(false);
		}
		...
		<div className='min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8'>
			<div className='max-w-md w-full space-y-8'>
				...
				<form className='mt-8 space-y-6' onSubmit={handleResetPassword}>
					...
					{message && <div className='text-green-500 text-sm bg-green-50 p-3 rounded-md'>{message}</div>}
